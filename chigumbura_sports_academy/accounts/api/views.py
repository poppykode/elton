...
def login(request):
    ...
    if not user.is_approve:
        return Response({'error': 'User is deactivated.'},status=HTTP_404_NOT_FOUND)
    token, _ = Token.objects.get_or_create(user=user)
    print('____________Token info')
    print(token)
    return Response({'token': token.key,'id': token.user_id},status=HTTP_200_OK)
